...
export default class Tools extends Plugin {
	...
	openFileExplorer = async (file: TFile | TFile[]) => {
		...
		try {
			...
			notice.noticeEl.addClass("red-background");
		}
	};

	treatFiles(files: TFile | TFile[], selectedPath: string) {
		if (Array.isArray(files) && files.length) {
			for (const file of files)
				this.MoveItem(file as TFile, selectedPath);
		} else {
			this.MoveItem(files as TFile, selectedPath);
		}
	}
	...
	async MoveItem(file: TFile, selectedPath: string) {
		...
		const { normalizedFullPath, fileName, destinationPath } =
			this.getDestinationPath(file as TFile, selectedPath);
	...
	async doCopy(
		...
		if (fileExists) {
			// file already exists
			if (!this.applyAll)
				new ConfirmModal(this.app, this, (result) => {
					if (result === 1) { //replace
						this.confirm(
							fileName,
							selectedPath,
						...
						if (this.files && this.files.length) {
							this.MoveItem(
								this.files.pop() as TFile,
								selectedPath
							);
						}
					} else if (result === 2) { // increment
						this.confirm(
							fileName,
							selectedPath,
							normalizedFullPath,
